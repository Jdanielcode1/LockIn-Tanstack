...
const LARGE_FILE_THRESHOLD = 100 * 1024 * 1024 // 100MB - use multipart upload
const MAX_FILE_SIZE = 5 * 1024 * 1024 * 1024 // 5GB - max upload size
...
export function VideoUpload({ projectId, onComplete, onCancel }: VideoUploadProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isLargeFile) {
        setUploadStatus('Initializing optimized upload...')
        setProgress(5)

        const workerUrl = import.meta.env.VITE_UPLOAD_ACTOR_URL
        if (!workerUrl) {
          throw new Error('Upload Worker URL not configured. Please set VITE_UPLOAD_ACTOR_URL.')
        }

        setUploadStatus('Uploading with dynamic optimization...')
        const result = await uploadLargeFile(
          videoFile,
          workerUrl,
          '', // uploaderId no longer used - kept for backward compatibility
          (progressInfo) => {
      ...
      // Get video metadata and generate thumbnail from start of video
      ...
      try {
        setUploadStatus('Generating thumbnail from video start...')
        ...
        // Generate thumbnail from the start of the video (0.5 seconds in to avoid black frames)
        ...
        console.error('Error getting video metadata or generating thumbnail:', error)
        // Continue without thumbnail - it's not critical
      }

      // Create timelapse record in database
      setUploadStatus('Creating timelapse record...')
      await createTimelapse({
        projectId,
        videoKey,
        durationMinutes: parseFloat(durationMinutes),
        isTimelapse: true, // All uploads are user-created timelapses now
        originalDuration,
        videoWidth,
        videoHeight,
        thumbnailKey,
  ...
  const handleFileChange = (file: File | null) => {
    if (file && (file.type === 'video/mp4' || file.type === 'video/quicktime' || file.type === 'video/x-m4v')) {
      ...
      <div className="bg-[#161b22] border border-[#30363d] rounded-md max-w-lg w-full mx-4 shadow-2xl">
        ...
        <div className="border-b border-[#30363d] px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-xl font-semibold text-[#c9d1d9]">Upload Your Timelapse</h2>
          </div>
          <button
            onClick={onCancel}
            disabled={uploading}
                ...
                {videoFile ? (
                  ...
                  <>
                    ...
                    <p className="text-[#c9d1d9] font-medium mb-1">
                      Drop your timelapse video here, or click to browse
                    </p>
                    <p className="text-sm text-[#8b949e]">
                      MP4, MOV up to 5GB
